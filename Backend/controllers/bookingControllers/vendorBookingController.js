  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const [userObj, vendorObj, serviceObj] = await Promise.all([
          User.findById(booking.userId).select('name phone email').lean(),
          Vendor.findById(vendorId).select('name phone email').lean(),
          Service.findById(booking.serviceId).select('title').lean()
        ]);

        if (userObj && serviceObj) {
          sendBookingEmails(booking, userObj, vendorObj, serviceObj)
            .catch(err => console.error('[AcceptBooking][bg] Email error:', err));
        }
      } catch (bgErr) {
        console.error('[AcceptBooking][bg] Background task failed:', bgErr);
      }
    });
