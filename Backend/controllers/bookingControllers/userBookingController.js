  ...
  try {
    ...
    if (!usePlanBenefits) {
      if (amount && amount > 0) {
        ...
        if (reqBasePrice != null && reqTax != null) {
          // Use breakdown provided by frontend
          basePrice = reqBasePrice || (service.basePrice || 500);
    ...
    setImmediate(async () => {
      try {
        ...
        if (wave1Vendors.length > 0) {
          ...
          const io = getIO();
          if (io) {
            io.to(userId.toString()).emit('booking_search_failed', {
              bookingId: bookingForBackground._id,
              message: 'No experts found nearby. The request was automatically cancelled.'
            });
          }
        }
...
/**
 * Delete booking (hide from user or hard delete if cancelled/completed)
 */
const deleteBooking = async (req, res) => {
  try {
    ...
    // Since we don't have a deletedByUser flag, we can either hard delete 
    // or add it. Let's just hard delete for now if it's cancelled
    // Alternatively, just delete it directly.
