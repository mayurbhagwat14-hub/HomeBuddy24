  ...
  try {
    ...
    res.status(200).json({
      ...
      homeContent: {
        ...
        isScrapVisible: homeContent.isScrapVisible ?? true,
...
const updateHomeContent = async (req, res) => {
  try {
    ...
    if (req.body.isScrapVisible !== undefined) homeContent.isScrapVisible = req.body.isScrapVisible;

    await homeContent.save();

    const formattedHomeContent = {
      id: homeContent._id,
      cityId: homeContent.cityId,
      banners: homeContent.banners,
      promos: homeContent.promos,
      ...
      isScrapVisible: homeContent.isScrapVisible
    };

    // Broadcast real-time update using socket
    const io = req.app.get('io');
    if (io) {
      io.emit('home_content_updated', formattedHomeContent);
    }

    res.status(200).json({
      success: true,
      message: 'Home content updated successfully',
      homeContent: formattedHomeContent
    });
  } catch (error) {
