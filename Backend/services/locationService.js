...
const findNearbyVendors = async (centerLocation, radiusKm = 10, filters = {}) => {
  ...
  try {
    ...
    try {
      ...
      if (hasGeoVendors > 0) {
        ...
        if (nearbyVendors.length === 0 && filters.city) {
          console.log(`[LocationService] 2dsphere found 0 vendors. Expanding search to entire city: ${filters.city}`);
          return await findVendorsByCity(filters.city, filters);
        }
    ...
    if (nearbyVendors.length === 0 && filters.city) {
      console.log(`[LocationService] Haversine found 0 vendors. Expanding search to entire city: ${filters.city}`);
      return await findVendorsByCity(filters.city, filters);
    }
