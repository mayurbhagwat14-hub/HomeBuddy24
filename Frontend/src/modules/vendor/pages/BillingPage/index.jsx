...
const BillingPage = () => {
  ...
  if (viewMode === 'select-services') {
    return (
      <div className="min-h-screen flex flex-col" style={{ background: themeColors.backgroundGradient }}>
        <div className="bg-white/80 backdrop-blur-xl sticky top-0 z-50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-b border-white">
          ...
          <div className="px-4 py-2 flex gap-2 overflow-x-auto scrollbar-hide">
            {serviceCategories.map(cat => (
              <button key={cat} onClick={() => setSelectedCategory(cat)}
                className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCategory === cat ? 'text-white shadow-lg' : 'bg-gray-100 text-gray-600'}`}
                style={selectedCategory === cat ? { background: themeColors.button, boxShadow: `0 4px 12px ${themeColors.button}40` } : {}}>{cat}</button>
            ))}
          </div>
        </div>
...
>>>>>>> c34bad6595e739083b7fdef20687f1c2dec39a86
          ...
          <button onClick={() => {
            ...
            setCurrentStep(2);
          }} className="flex-[2] py-3.5 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg"
...
>>>>>>> c34bad6595e739083b7fdef20687f1c2dec39a86
  ...
  return (
    <div className="min-h-screen pb-0 flex flex-col" style={{ background: themeColors.backgroundGradient }}>
      ...
      <div className="sticky top-0 z-50 bg-white">
        ...
        <div className="px-4 py-4 border-b border-gray-50 flex justify-between relative overflow-hidden shadow-sm">
          {[
            ...
            return (
              <button key={step.id} onClick={() => isReached && setCurrentStep(step.id)}
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${(isActive || isCompleted) ? 'text-white shadow-lg shadow-blue-200' : 'bg-gray-100 text-gray-400'} ${isActive ? 'ring-4 ring-blue-50' : ''}`}
                     style={(isActive || isCompleted) ? { background: themeColors.button } : {}}>
          ...
          <div className="absolute top-8 left-0 right-0 h-0.5 bg-gray-200 -z-0 mx-8">
            <div className="h-full transition-all duration-300" style={{ background: themeColors.button, width: `${((maxStep - 1) / 4) * 100}%` }}></div>
          </div>
        </div>
      </div>
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900 flex items-center gap-2 mb-2 pb-2 border-b border-gray-100">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${paymentMode === 'cash' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
                        {paymentMode === 'cash' ? <BiRupee /> : <MdQrCode />}
                      </span>
                      Payment Method
                    </h4>
...
>>>>>>> c34bad6595e739083b7fdef20687f1c2dec39a86
        {currentStep === 1 && (
          <button 
            ...
            className="w-full py-3.5 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg"
            style={{ background: themeColors.button, boxShadow: `0 4px 12px ${themeColors.button}40` }}
          >
            Next: Parts <FiArrowRight className="w-5 h-5" />
          </button>
        ...
        {currentStep === 2 && (
          <>
            <button onClick={() => setCurrentStep(1)} className="flex-1 py-3.5 bg-white text-gray-700 font-bold rounded-xl border border-gray-200">Back</button>
            <button 
        ...
        {currentStep === 3 && (
          <>
            <button onClick={() => setCurrentStep(2)} className="flex-1 py-3.5 bg-white text-gray-700 font-bold rounded-xl border border-gray-200">Back</button>
            <button 
        ...
        {currentStep === 4 && (
          <>
            <button onClick={() => setCurrentStep(3)} className="flex-1 py-3.5 bg-white text-gray-700 font-bold rounded-xl border border-gray-200">Back</button>
            <button 
              onClick={() => setCurrentStep(5)} 
              className="flex-[2] py-3.5 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg"
              style={{ background: themeColors.button, boxShadow: `0 4px 12px ${themeColors.button}40` }}
            >
              Review Bill <FiArrowRight className="w-5 h-5" />
            </button>
          </>
        )}
        {currentStep === 5 && (
          <>
            <button
              ...
              className="flex-1 py-3.5 bg-white text-gray-700 font-bold rounded-xl border border-gray-200 disabled:opacity-50"
            >
              Back
            </button>
            ...
            <div className="flex-[2] grid grid-cols-2 gap-3">
              ...
              {(isOtpSent && paymentMode === 'cash') ? (
                <button
                  ...
                  className="py-3 text-white font-bold rounded-xl shadow-lg flex flex-col items-center justify-center gap-1 active:scale-95 transition-all text-[10px]"
                ...
                <button
                  ...
                  className="py-3 text-white font-bold rounded-xl shadow-lg flex flex-col items-center justify-center gap-1 active:scale-95 transition-all disabled:opacity-50 text-[10px]"
                  style={{ background: themeColors.button }}
                >
                  {otpLoading ? <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /> : <BiRupee className="w-5 h-5" />}
                  Collect Cash
                </button>
              )}
              ...
              <button
                ...
                disabled={submitting || qrLoading || otpLoading}
                className="py-3 text-white font-bold rounded-xl shadow-lg flex flex-col items-center justify-center gap-1 active:scale-95 transition-all disabled:opacity-50 text-[10px]"
                style={{ background: themeColors.button }}
              >
                {qrLoading ? <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /> : <MdQrCode className="w-5 h-5" />}
                Online Payment
              </button>
            </div>
          </>
