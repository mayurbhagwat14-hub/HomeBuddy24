...
import { FiMapPin, FiClock, FiUser, FiPhone, FiNavigation, FiArrowRight, FiEdit, FiCheckCircle, FiCreditCard, FiX, FiCheck, FiTool, FiXCircle, FiAward, FiPackage, FiAlertCircle, FiFileText } from 'react-icons/fi';
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: themeColors.backgroundGradient }}>
      ...
      <main className="px-4 py-6">
        {/* Modernized Combined Header Card for Service & User */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white/80 backdrop-blur-xl rounded-3xl p-5 mb-5 border border-white/50 relative overflow-hidden"
          style={{
            boxShadow: '0 10px 40px -10px rgba(0,0,0,0.08)',
          }}
        >
          {/* Decorative Gradient Blob */}
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-100 rounded-full mix-blend-multiply filter blur-2xl opacity-50 -translate-y-1/2 translate-x-1/3"></div>

          <div className="flex items-start justify-between mb-5 relative z-10 border-b border-gray-100/60 pb-5">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 flex items-center gap-1.5">
                <FiTool className="w-3 h-3" /> Service Details
              </span>
            ...
            <div className="flex flex-col items-end gap-2">
              <span
                ...
                style={{
                  background: `linear-gradient(135deg, ${themeColors.button}15, ${themeColors.button}30)`,
                  color: themeColors.button,
                  border: `1px solid ${themeColors.button}20`
                }}
              >
                {booking.status.replace('_', ' ')}
              </span>
              {booking.assignedTo?.name === 'You (Self)' && (
                <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200/60 shadow-sm uppercase tracking-widest flex items-center gap-1">
                  <FiCheckCircle className="w-3 h-3" /> Personal Job
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-4">
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner relative overflow-hidden"
                style={{ background: `linear-gradient(135deg, ${themeColors.icon}10, ${themeColors.icon}20)` }}
              >
              ...
              <div className="flex flex-col">
                ...
                <p className="font-bold text-gray-900 text-base">{booking.user?.name || booking.customerName || 'Customer'}</p>
                <p className="text-sm font-medium text-gray-500 flex items-center gap-1.5">
                  <FiPhone className="w-3 h-3" /> {booking.user?.phone || booking.customerPhone || 'Hidden'}
                </p>
              </div>
            </div>
            <button
              onClick={handleCallUser}
              className="w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-90 hover:shadow-lg group"
              style={{ background: `linear-gradient(135deg, ${themeColors.button}, ${themeColors.icon})`, boxShadow: `0 8px 16px -4px ${themeColors.button}60` }}
            >
              <FiPhone className="w-5 h-5 text-white group-hover:animate-pulse" />
            </button>
          </div>
        </motion.div>

        {/* Modern Address & Location Card */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1 }}
          className="bg-white/90 backdrop-blur-md rounded-3xl p-5 mb-5 relative overflow-hidden group"
          style={{ boxShadow: '0 10px 30px -5px rgba(0,0,0,0.06)' }}
        >
          <div className="flex items-start gap-4 mb-5 relative z-10">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 bg-blue-50 text-blue-600 shadow-inner border border-blue-100/50">
              <FiMapPin className="w-6 h-6" />
            </div>
            <div className="flex-1 pt-1">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Service Location</p>
              <p className="font-bold text-gray-800 leading-snug text-sm">{booking.location.address}</p>
              <div className="flex items-center gap-1.5 mt-2">
                <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]"></span>
                <p className="text-xs font-bold text-blue-600 tracking-wide">{booking.location.distance} away</p>
              </div>
            </div>
          </div>
          ...
          {/* Elegant Map Embed */}
          <div 
            className="w-full h-40 rounded-2xl overflow-hidden mb-5 bg-gray-100 relative cursor-pointer border-2 border-white shadow-inner transform transition-transform group-hover:scale-[1.02] duration-300" 
              ...
              return (
                <>
                  <iframe
                    ...
                    height="100%"
                    frameBorder="0"
                    style={{ border: 0, pointerEvents: 'none', filter: 'contrast(1.05) saturate(1.1)' }}
                    src={`https://maps.google.com/maps?q=${mapQuery}&z=15&output=embed`}
                    allowFullScreen
                    tabIndex="-1"
                  ></iframe>
                  {/* Premium Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent transition-colors flex items-end justify-center pb-3">
                    <span className="bg-white/95 backdrop-blur px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest text-gray-800 shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 flex items-center gap-2">
                      <FiMapPin className="w-3 h-3 text-blue-600" /> Tap to expand map
                    </span>
                  </div>
                </>
              );
          ...
          <div className="flex gap-3 relative z-10">
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 bg-white border border-gray-200/80 shadow-sm hover:bg-gray-50 hover:border-gray-300 text-gray-700"
            >
            ...
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl font-bold text-white flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg relative overflow-hidden group"
              style={{
                background: `linear-gradient(135deg, ${themeColors.button}, ${themeColors.icon})`,
                boxShadow: `0 8px 20px -6px ${themeColors.button}80`,
              }}
            >
              <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"></div>
              <FiNavigation className="w-4 h-4 relative z-10" />
        ...
        {(() => {
          ...
          return (
            <motion.div 
              ...
              className="bg-gradient-to-br from-orange-50/80 to-amber-50/40 backdrop-blur-sm rounded-3xl p-5 mb-5 border border-orange-100/60 relative overflow-hidden"
            >
              ...
              <p className="text-[10px] font-bold text-orange-600/80 uppercase tracking-widest mb-2 relative z-10 flex items-center gap-1.5">
                <FiFileText className="w-3 h-3" /> Description & Notes
              </p>
              <p className="text-sm text-gray-800 leading-relaxed font-medium relative z-10">{displayDesc}</p>
            </motion.div>
          );
        })()}
        ...
        {booking.items && booking.items.length > 0 && (
          <motion.div 
            ...
            className="bg-white/90 backdrop-blur-md rounded-3xl p-5 mb-5 relative overflow-hidden"
            style={{ boxShadow: '0 10px 30px -5px rgba(0,0,0,0.06)' }}
          >
            ...
            {/* Service Category */}
            <div className="flex items-center gap-4 mb-4 bg-gray-50/50 p-3 rounded-2xl border border-gray-100/60">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm"
                style={{ backgroundColor: `white`, border: `1px solid ${themeColors.button}20` }}>
                {booking.categoryIcon ? (
                  <img src={booking.categoryIcon} alt="" className="w-6 h-6 object-contain" />
                ) : (
                  <FiTool className="w-5 h-5" style={{ color: themeColors.button }} />
                )}
              </div>
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Service Category</p>
                <p className="text-sm font-bold text-gray-900">{booking.serviceCategory || booking.serviceType || 'Service'}</p>
              </div>
            </div>
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center gap-4 mb-4 pt-4 border-t border-dashed border-gray-200/60">
            ...
            <div className="pt-4 border-t border-dashed border-gray-200/60 space-y-3">
              ...
              {booking.items.map((item, index) => (
                <div key={index} className="flex justify-between items-start bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex-1 min-w-0 pr-3">
                    <div className="flex items-start gap-2 mb-1">
                      <span className="text-xs font-black px-2 py-0.5 rounded-md mt-0.5"
                        style={{ color: themeColors.button, backgroundColor: `${themeColors.button}10` }}>
                        {item.quantity}x
                      </span>
                      <span className="text-sm font-bold text-gray-800 leading-snug">{item.card?.title || 'Service Item'}</span>
                    </div>
                    {item.card?.subtitle && <p className="text-[11px] text-gray-500 mt-1 pl-8 line-clamp-2 leading-relaxed">{item.card.subtitle}</p>}
                    {item.card?.duration && <p className="text-[10px] font-bold text-gray-400 mt-1.5 pl-8 flex items-center gap-1"><FiClock className="w-3 h-3" /> {item.card.duration}</p>}
                  </div>
                  <div className="text-right shrink-0 flex flex-col items-end justify-center">
                    <p className="text-sm font-black text-gray-900 tracking-tight">₹{((item.card?.price || 0) * (item.quantity || 1)).toLocaleString()}</p>
                    {item.quantity > 1 && <p className="text-[10px] font-bold text-gray-400 mt-0.5">₹{item.card?.price || 0} /ea</p>}
                  </div>
                </div>
              ))}
              <div className="flex justify-between items-center p-4 mt-2 bg-gray-50 rounded-2xl border border-gray-100/80">
                <p className="text-sm font-bold text-gray-600 uppercase tracking-wide">Total Base Price</p>
                <p className="text-lg font-black" style={{ color: themeColors.button }}>₹{(booking.basePrice || 0).toFixed(2)}</p>
              </div>
            </div>
          </motion.div>
        )}

        {/* Modern Time Slot Card */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
          className="bg-gradient-to-br from-indigo-50/80 to-purple-50/40 backdrop-blur-sm rounded-3xl p-5 mb-6 border border-indigo-100/60 relative overflow-hidden"
        >
          ...
          <div className="flex items-center gap-4 relative z-10">
            <div className="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center shrink-0 border border-indigo-100/50">
              <FiClock className="w-6 h-6 text-indigo-500" />
            </div>
            <div className="flex-1">
              <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Scheduled For</p>
              <p className="text-base font-black text-gray-900 mb-0.5">{booking.timeSlot.date}</p>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-100/50">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
                <p className="text-xs font-bold text-indigo-700">{booking.timeSlot.time}</p>
              </div>
            </div>
          </div>
        </motion.div>
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col gap-3 w-full">
                <button
                  ...
                  style={{ background: themeColors.button, boxShadow: `0 4px 12px ${themeColors.button}40` }}
                >
