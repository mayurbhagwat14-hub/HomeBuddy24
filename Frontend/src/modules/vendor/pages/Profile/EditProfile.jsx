...
const EditProfile = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!validationResult.success) {
      ...
      const firstIssue = validationResult.error?.issues?.[0] || validationResult.error?.errors?.[0];
      const errorMessage = firstIssue ? `${firstIssue.path?.[0] || 'Field'}: ${firstIssue.message}` : 'Validation failed';
