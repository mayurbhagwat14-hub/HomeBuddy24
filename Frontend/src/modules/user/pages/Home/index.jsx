...
const Home = () => {
  ...
  // Listen for real-time Home Content updates (e.g. Scrap Toggle)
  useEffect(() => {
    if (socket) {
      const handleHomeUpdate = (data) => {
        setHomeContent(prev => {
          if (!prev) return prev;
          return { ...prev, ...data };
        });
      ...
      socket.on('home_content_updated', handleHomeUpdate);
      
      return () => {
        socket.off('home_content_updated', handleHomeUpdate);
      };
    }
  }, [socket]);
...
>>>>>>> c34bad6595e739083b7fdef20687f1c2dec39a86
              ...
              {homeContent?.isCategorySectionsVisible !== false && (homeContent?.categorySections || []).sort((a, b) => (a.order || 0) - (b.order || 0)).map((section, sIdx) => (
                <motion.div key={section._id || sIdx} variants={itemVariants} initial="hidden" animate="visible">
                  <Suspense fallback={<div className="h-40 bg-gray-50 animate-pulse rounded-xl mx-4" />}>
