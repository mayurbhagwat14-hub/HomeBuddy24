...
import { FiArrowLeft, FiMapPin, FiClock, FiCheckCircle, FiXCircle, FiLoader, FiCalendar, FiPackage, FiEdit2, FiPhone, FiMail, FiKey, FiStar, FiAward, FiX, FiUser, FiChevronRight, FiSearch, FiHome, FiAlertCircle } from 'react-icons/fi';
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.data?.success && response.data?.settings) {
          ...
          setSupportInfo({
            email: supportEmail || 'help@HomeBuddy24.in',
  ...
  return (
    <div className="min-h-[100dvh] pb-32 relative bg-[#F8FAFC] overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        <header className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 border-b border-white px-4 py-4 flex items-center justify-between shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate('/user/my-bookings')}
              className="w-10 h-10 bg-white/80 rounded-xl flex items-center justify-center shadow-sm border border-white"
            >
        ...
        <main className="max-w-xl mx-auto px-4 py-6 space-y-6">
              ...
              <div className="relative z-10 p-6">
                ...
                {booking.paymentStatus !== 'success' && (
                  <>
                    ...
                    >
                      <BiRupee className="w-4 h-4 group-hover:rotate-12 transition-transform" />
                      Pay Online Now
                  ...
                  <div className="space-y-4">
                    ...
                    {(booking.paymentMethod || booking.paymentStatus === 'success') && (
                      <div className="mt-2 pt-2 border-t border-gray-100">
                        <div className="flex justify-between text-xs font-bold text-gray-600">
                          <span className="flex items-center gap-2 uppercase tracking-wide">
                            {booking.paymentMethod === 'cash collected' ? <BiRupee className="text-emerald-500" /> : <MdQrCode className="text-blue-500" />}
                            Payment Method
                          </span>
            ...
            <div className="bg-white rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-6 space-y-4">
              <div className="text-center mb-4">
                <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-3">
                  <BiRupee className="w-8 h-8 text-orange-600" />
                </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              onClick={() => {
                const email = supportInfo.email || 'help@HomeBuddy24.in';
                const link = document.createElement('a');
