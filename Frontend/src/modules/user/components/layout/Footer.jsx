...
const Footer = () => {
  ...
  const defaultEmail = `support@${appName.toLowerCase().replace(/\s+/g, '')}.com`;
  ...
  const footerSections = [
    {
      title: 'Company',
      links: [
        { label: 'About Us', path: '/user/about-HomeBuddy24' },
    ...
    },
    {
      ...
      links: [
        { 
          label: settings?.supportEmail || settings?.companyEmail || defaultEmail, 
          path: `mailto:${settings?.supportEmail || settings?.companyEmail || defaultEmail}`, 
          icon: FiMail 
        },
        { 
      ...
      <div className="max-w-screen-xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            <Link to="/user/home" className="inline-block transform hover:scale-105 transition-transform duration-300">
              <Logo className="h-10 w-auto" />
            </Link>
            <p className="text-gray-500 text-sm leading-relaxed max-w-xs">
              {settings?.companyName || appName} is your one-stop destination for all home services. From electrical repairs to premium salon services, we bring the experts to your doorstep.
            </p>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-gray-400 text-sm">
            © {currentYear} {settings?.companyName || appName}. All rights reserved.
          </p>
