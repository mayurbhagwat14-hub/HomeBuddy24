...
import { FiSettings, FiGrid, FiSave, FiUser, FiMail, FiTrash2, FiPlus, FiUsers, FiShield, FiFileText, FiMapPin, FiPhone, FiHeadphones, FiMessageCircle, FiEdit, FiLock, FiUnlock, FiX } from 'react-icons/fi';
      ...
      try {
        ...
        if (res.success && res.settings) {
          setFinancialSettings({
            ...
            storeCommissionPercentage: res.settings.storeCommissionPercentage ?? 10,
      ...
      {isSuperAdmin && (
        <div onClick={() => setActiveView('branding')}
          ...
          <div className="w-12 h-12 bg-pink-50 rounded-lg flex items-center justify-center mb-4 group-hover:bg-pink-100 transition-colors">
            <FiEdit className="w-6 h-6 text-pink-600" />
          </div>
          <h3 className="text-lg font-bold text-gray-800 mb-2">App Branding</h3>
          <p className="text-sm text-gray-500">Update app name, logo, and visual identity</p>
        </div>
      )}
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 h-fit">
                ...
                <form onSubmit={handleFinancialSave} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Store Commission (%)</label>
                      <input type="number" name="storeCommissionPercentage" value={financialSettings.storeCommissionPercentage} onChange={handleFinancialChange}
                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-green-500 transition-all" />
                      <p className="text-[10px] text-gray-400 mt-1">Commission charged on store orders</p>
                    </div>
                    <div>
