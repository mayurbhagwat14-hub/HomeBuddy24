  ...
  const syncHomeToBackend = async (homeData) => {
    ...
    try {
      const payload = {
        ...
        isScrapVisible: homeData.isScrapVisible
      };
        ...
        <div className="space-y-4">
          <div>
            <div className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-3">
              ...
              <span>Scrap Feature Settings</span>
            </div>
            <p className="text-sm text-gray-500 mb-4">
              Toggle the Scrap promotion banner on the User App Home Screen. Changes are applied in real-time.
            </p>
          </div>
          <div className="flex items-center mb-3 gap-4">
            <ToggleSwitch
              label="Show Scrap Banner"
              checked={home?.isScrapVisible !== false}
              onChange={() => patchHome({ isScrapVisible: !home?.isScrapVisible })}
            />
          </div>
        </div>
      </CardShell>
